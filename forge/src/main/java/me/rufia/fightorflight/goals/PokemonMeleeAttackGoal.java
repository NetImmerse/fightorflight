...
public class PokemonMeleeAttackGoal extends MeleeAttackGoal {
    ...
    public void tick() {
        ...
        Entity shootTarget = this.mob.getTarget();
    ...
    public boolean shouldFightTarget(){
        ...
        LivingEntity owner = pokemonEntity.getOwner();
        if (owner != null){ // IF POKEMON HAS OWNER
            ...
            if (this.mob.getTarget() != null){ // if pokemon has a target
            if (CobblemonFightOrFlight.getRangedAttacker(pokemonEntity) //if pokemon is a ranged attacker
                    && this.mob.getTarget().distanceToSqr(this.mob) >= 9.0F) { // and distance to target is closer than 3 blocks
                return false;
            }}
            ...
            if (this.mob.getTarget() != null){
                ...
                if (CobblemonFightOrFlight.getRangedAttacker(pokemonEntity) // if pokemon is a ranged attacker
                        && this.mob.distanceToSqr(targetEntity) >= 4.0F //and target is farther than 3 blocks
                        && CobblemonFightOrFlight.getFightOrFlightCoefficient(pokemonEntity) < 40.0F ) {  // and FoF coeff. is < 25
                     return false;
                }
